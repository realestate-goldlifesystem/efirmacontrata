...
function procesarRegistroParte2(datos) {
  ...
  try {
    ...
    switch (datos.tipoRegistro.tipo) {
      case 'TIPO_1_1':
        Logger.log('🏢 Ejecutando TIPO 1.1: Nuevo inmueble mismo propietario');
        procesarTipo11_NuevoInmueble(sheet, row, datos);
        break;
        
      case 'TIPO_1_2':
        Logger.log('🔄 Ejecutando TIPO 1.2: Renovación');
        procesarTipo12_Renovacion(sheet, row, datos);
        break;
        
      case 'TIPO_1_3':
        Logger.log('🔀 Ejecutando TIPO 1.3: Cambio de tipo de negocio');
        procesarTipo13_CambioTipoNegocio(sheet, row, datos);
        break;
        
      case 'TIPO_4':
        Logger.log('🆕 Ejecutando TIPO 4: Nuevo propietario');
...
function procesarTipo11_NuevoInmueble(sheet, row, datos) {
  ...
  actualizarEstadoFinal(sheet, row, 'TIPO_1_1');
  
  Logger.log('✅ TIPO 1.1 completado');
}
...
function procesarTipo4_NuevoPropietario(sheet, row, datos) {
  ...
  actualizarEstadoFinal(sheet, row, 'TIPO_4');
...
function procesarTipo12_Renovacion(sheet, row, datos) {
  ...
  if (filaOriginal > 0) {
    actualizarEstadoFinal(sheet, filaOriginal, 'TIPO_1_2');
  }
  
  Logger.log('✅ TIPO 1.2 completado');
}
...
function procesarTipo13_CambioTipoNegocio(sheet, row, datos) {
  ...
  // 10. BORRAR fila temporal
  Logger.log(`🗑️ Borrando fila temporal: ${row}`);
  borrarFilaTemporal(sheet, row);
  
  // 11. Actualizar estado final en fila ORIGINAL
  if (filaOriginal > 0) {
    actualizarEstadoFinal(sheet, filaOriginal, 'TIPO_1_3');
  }
  
  Logger.log('✅ TIPO 1.3 completado');
}
